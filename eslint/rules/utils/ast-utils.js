...
function equalTokens(left, right, sourceCode) {
  ...
  for(let i = 0; i < tokensL.length; ++i) {
    if(tokensL[i].type !== tokensR[i].type || tokensL[i].value !== tokensR[i].value) return false;
  }
  return true;
}
...
module.exports = {
  ...
  isDefaultThisBinding(node, sourceCode, { capIsConstructor = true } = {}) {
     ...
     */
    if(node.parent.type === "PropertyDefinition" && node.parent.value === node) return false;
    // Class static blocks are implicit functions. In this case, `this` is always not default binding.
    if(node.type === "StaticBlock") return false;
    if((capIsConstructor && isES5Constructor(node)) ||hasJSDocThisTag(node, sourceCode)) return false;
    ...
    while(currentNode) {
      ...
      switch(parent.type) {
        ...
        case "ReturnStatement": {
          ...
          if(func === null || !isCallee(func)) return true;
          currentNode = func.parent;
          break;
        }
        case "ArrowFunctionExpression":
          if(currentNode !== parent.body || !isCallee(parent)) return true;
  ...
  getNameLocationInGlobalDirectiveComment(sourceCode, comment, name) {
    ...
    const end = { line: start.line, column: start.column + (match ? name.length : 1) };
